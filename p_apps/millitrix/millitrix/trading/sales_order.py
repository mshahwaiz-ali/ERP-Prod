...
from millitrix.utils.doctype_ids import SALES_ORDER
...
from millitrix.utils.order_balance import SIDE_SALES, validate_split_child_order
...
def validate(doc, method=None):
	...
	if not doc.doctypeid:
		doc.doctypeid = SALES_ORDER
	validate_fiscal_period(doc.sodate)
	if not doc.status:
		doc.status = order_status_label("IN")
	validate_split_child_order(doc, SIDE_SALES)
	doc.amount = calc_order_amount(doc)
	doc.receivable = doc.amount
...
def on_cancel(doc, method=None):
	if frappe.utils.flt(doc.truckissued) > 0:
		frappe.throw(_("Cannot cancel Sales Order with issued trucks"))
	mark_unposted(doc)
	doc.db_set("status", order_status_label("CA"))
