...
// Oracle SOCancel.fmb parity — open SO lines, balance qty, cancel validation.

frappe.ui.form.on("SO Cancellation", {
	onload(frm) {
	...
	refresh(frm) {
		...
		frm.set_query("sonumber", "details", () => ({
			query: "millitrix.utils.so_cancellation_form.search_open_sales_orders",
			filters: {
	...
	partyid(frm) {
		if (frm.doc.details?.length) {
			frappe.show_alert({
				message: __("Party changed — verify SO lines still match"),
				indicator: "orange",
			});
		}
	},
...
frappe.ui.form.on("SO Cancellation Detail", {
	sonumber(frm, cdt, cdn) {
	...
	cancelqty(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!row.sonumber || !flt(row.cancelqty)) {
			return;
		}
