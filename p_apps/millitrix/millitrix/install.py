...
def after_install():
	...
	if frappe.db.exists("Installed Application", {"app_name": "millitrix"}):
		name = frappe.db.get_value("Installed Application", {"app_name": "millitrix"}, "name")
		frappe.db.set_value(
			"Installed Application",
			name,
			{"has_setup_wizard": 0, "is_setup_complete": 1},
		)
...
def cleanup_old_mill_module():
	...
	old_doctypes = frappe.get_all("DocType", filters={"module": "Mill ERP"}, pluck="name")
