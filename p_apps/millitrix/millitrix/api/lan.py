...
def ensure_lan_hostnames() -> dict:
	"""Register millitrix.local + current LAN IP on site (other devices use IP only)."""
	...
	domains -= stale
	domains.update({local_host, site_config.get("host_name") or ""})
	if ip:
		domains.add(ip)
	domains.discard("")
...
def _is_trusted_server_request() -> bool:
	"""LAN details are only exposed on the server login host (millitrix.local / localhost)."""
	host = (frappe.get_request_header("Host") or "").split(":")[0].strip().lower()
	local_host = get_local_host().lower()
	return host in {local_host, "localhost", "127.0.0.1"}
