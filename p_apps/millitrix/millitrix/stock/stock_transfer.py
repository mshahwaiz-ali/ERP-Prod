...
def on_cancel(doc, method=None):
	for line in doc.details:
		...
		if doc.itemcode and grain_qty > 0:
			src = _grain_key(doc, line)
			apply_stock_out(dest_key, grain_qty, movement_date=doc.tdate, check_reserved=False)
			apply_stock_in(src, grain_qty, rate=flt(line.rate), movement_date=doc.tdate)
		...
		if line.bagid and flt(line.bagqty) > 0:
			...
			if src_b and dst_b:
				bqty = flt(line.bagqty)
				apply_stock_out(dst_b, bqty, movement_date=doc.tdate, check_reserved=False)
				apply_stock_in(
					src_b,
					bqty,
					rate=flt(line.bagrate),
