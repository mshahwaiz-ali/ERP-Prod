// Login page — server PC uses local.mill; copy LAN IP for phone/other laptops.
...
(function () {
	...
	const is_server_host = () => {
		...
		return host === "local.mill" || host === "127.0.0.1" || host === "localhost";
	};
	...
	frappe.ready(() => {
		// Only on server laptop (local.mill). Mobile opens IP directly — no extra box.
		if (!is_server_host()) {
			return;
		}
